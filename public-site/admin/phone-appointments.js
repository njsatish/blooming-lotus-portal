...
(() => {
  ...
  /* BLOOMING_PAST_TIME_HELPERS_V21_52_1_START */
  const roanokeClock=()=>{const parts=new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(new Date());return Object.fromEntries(parts.map(x=>[x.type,x.value]));};
  const roanokeDate=()=>{const p=roanokeClock();return `${p.year}-${p.month}-${p.day}`;};
  const nextQuarterHour=()=>{const p=roanokeClock(),now=Number(p.hour)*60+Number(p.minute);return Math.ceil(now/15)*15;};
  const isFutureSlot=(date,time)=>date!==roanokeDate()||toMinutes(time)>=nextQuarterHour();
  /* BLOOMING_PAST_TIME_HELPERS_V21_52_1_END */
  ...
  function loadTimes(){
    ...
    const selectedDate=$('ph-date')?.value;const options=(data.slots||[]).filter(slot=>isFutureSlot(selectedDate,slot.time)&&!slot.fullyBooked&&(slot.therapists||[]).includes(therapist)).map(slot=>slot.time);setTimes(options);message(options.length?`${options.length} available times for ${therapist}.`:selectedDate===roanokeDate()?`No future times remain for ${therapist} today.`:`No available times remain for ${therapist} on this date.`,!options.length);
  }
